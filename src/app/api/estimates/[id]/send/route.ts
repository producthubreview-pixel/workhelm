...
export async function PATCH(
  ...
  // The lead is either the estimate's direct link (leadId) or the customer's
  // originating lead.
  const linkedLeadId = updated.customer?.lead?.id ?? updated.leadId;
  if (linkedLeadId) {
    await db.lead.update({ where: { id: linkedLeadId }, data: { status: "ESTIMATE_SENT" } });
  }
